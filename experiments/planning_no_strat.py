...
def run_fixed_questions_experiment(
    ...
    print("\n=== Pre-filtering categories for persona relevance ===")
    relevant_categories_pool = []
    for category in selected_categories:
        is_relevant, max_score, cached_scores = is_category_relevant_for_persona(
            category, persona_index, min_score_threshold
        )
        if is_relevant:
            print(f"  ✓ Category '{category}' is relevant (Max Score: {max_score:.1f}). Added to pool.")
            relevant_categories_pool.append({'category': category, 'cached_scores': cached_scores})
        if len(relevant_categories_pool) >= target_successful_episodes:
            print(f"\nFound enough relevant categories ({len(relevant_categories_pool)}) for the experiment.")
            break
            
    if len(relevant_categories_pool) < target_successful_episodes:
        print("\n" + "="*60)
        print(f"FATAL ERROR: Could not find enough relevant categories for persona {persona_index}.")
        print(f"Found {len(relevant_categories_pool)}, but require {target_successful_episodes}.")
        print("Please try a different seed or lower the min_score_threshold.")
        print("="*60)
        return {
            'error': 'Insufficient relevant categories',
            'found': len(relevant_categories_pool),
            'required': target_successful_episodes
        }
    ...
    print(f"\n=== Starting Episode Execution with {target_successful_episodes} Pre-Filtered Relevant Categories ===")
    
    for planned_episode_index in range(1, target_successful_episodes + 1):
        episode_data = relevant_categories_pool[planned_episode_index - 1]
        category = episode_data['category']
        cached_scores = episode_data['cached_scores']
        ...
        print(f"\n--- Running Episode {planned_episode_index}/{target_successful_episodes} (Category: {category}, Type: {episode_type}) ---")
        ...
        try:
            ...
            episode_num_for_logging = planned_episode_index
            metrics_wrapper = MetricsWrapper(env, 
                                           output_path=os.path.join(output_dir, f"episode_{episode_num_for_logging}.jsonl"))
            ...
            while not terminated and not truncated and step_count <= 21:
                ...
                if info['action_type'] == 'ask':
                    if env.dialog_history:
                        ...
                        if isinstance(last_qa, tuple) and len(last_qa) >= 2:
                            print(f"  Step {step_count}: Asked Q{len(env.dialog_history)}: {last_qa[0]}")
                            print(f"  Step {step_count}: Received A{len(env.dialog_history)}: {last_qa[1]}")
                    ...
                    if is_tracking and len(env.dialog_history) <= 10:
                        ...
                        best_id, best_score = env.oracle_scores[0]
                        regret = max(0.0, best_score - chosen_score)
                        
                        ordered_product_ids = [pid for pid, _ in env.oracle_scores]
                        chosen_rank = ordered_product_ids.index(chosen_product_id) + 1 if chosen_product_id in ordered_product_ids else None
                        
                        print(f"    Forced recommendation after Q{len(env.dialog_history)} (Regret: {regret:.1f}, Rank: {chosen_rank})")
            ...
            product_info = {'num_products': len(env.products), 'products_with_scores': []}
            if hasattr(env, 'products') and hasattr(env, 'oracle_scores'):
                ...
                for product_id, avg_score in env.oracle_scores:
                    ...
                    if product:
                        product_info['products_with_scores'].append({'id': product_id, 'name': product.get('title', 'Unknown'), 'price': product.get('price', 'Unknown'), 'average_score': float(avg_score)})
            
            if current_info.get('action_type') == 'recommend' and 'chosen_score' in current_info:
                ordered_product_ids = [pid for pid, _ in env.oracle_scores]
                chosen_rank = ordered_product_ids.index(current_info.get('chosen_product_id')) + 1 if current_info.get('chosen_product_id') in ordered_product_ids else None
                current_info['chosen_rank'] = chosen_rank
                current_info['total_products'] = len(env.products)

                episode_result = {
                    'episode': planned_episode_index, 'category': category, 'persona_index': persona_index,
                    'is_tracking_episode': is_tracking, 'steps': step_count, 'terminated': terminated, 'truncated': truncated,
                    'final_info': current_info, 'full_dialog': full_dialog, 'product_info': product_info,
                    'regret_progression': regret_progression, 'confidence_progression': confidence_progression,
                    'rank_progression': rank_progression, 'planned_episode_index': planned_episode_index,
                ...
                print(f"  Episode {planned_episode_index}: Successfully completed (Regret: {current_info.get('regret', 0):.1f})")
            else:
                print(f"  Episode {planned_episode_index}: Failed - No final recommendation made.")

            metrics_wrapper.close()

        except Exception as e:
            print(f"  Error in episode {planned_episode_index} for category {category}: {e}")
            continue
            
    print(f"\n=== Results Analysis ===")
    print(f"Target successful episodes: {target_successful_episodes}")
    print(f"Actual successful episodes: {len(all_results)}")
