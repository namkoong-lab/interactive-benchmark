#!/usr/bin/env python3
"""
Runner script for Persona Alignment Experiment

Temporary experiment - this entire folder can be deleted after completion.

Usage:
    python -m experiments.persona_alignment.run_experiment --config config.yaml
    python -m experiments.persona_alignment.run_experiment --experiment_type variable_category --total_trajectories 5
"""
...
from experiments.persona_alignment.persona_alignment_experiment import PersonaAlignmentExperiment


def main():
    parser = argparse.ArgumentParser(
        description="Persona Alignment Experiment - Measures alignment between persona responses and descriptions",
...
Examples:
  ...
  # With custom evaluator model
  %(prog)s --config config.yaml --evaluator_model claude-sonnet-4-20250514
        """
    )
    ...
    parser.add_argument("--evaluator_model", default="claude-sonnet-4-20250514",
                       help="Claude model for alignment evaluation (default: claude-sonnet-4-20250514)")
    ...
    experiment = PersonaAlignmentExperiment(config, evaluator_model=args.evaluator_model)
    experiment.run()
    
    print("\n✅ Persona Alignment Experiment completed successfully!")
