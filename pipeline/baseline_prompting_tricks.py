#!/usr/bin/env python3
"""
Baseline Experiment: Prompting tricks + Everything in context (with Checkpointing).

This script runs the full experimental setup with an LLM-based agent that uses
a Chain-of-Thought style prompt. Its structure is identical to experiment1.py 
for fair comparison, and it includes checkpointing to save and resume progress.
"""
...
class PromptingTricksAgent:
    """
    An LLM-based agent that asks a fixed number of meaningful questions,
    then uses an advanced prompt for the final recommendation.
    This agent is STATELESS and does not learn between episodes.
    """
    ...
    def __init__(self, model: str = "gpt-4o", max_questions: int = 3):
        ...
        self.last_response = ""  # Will store the generated question or final rationale
        # Compatibility attributes for checkpointing structure
    ...
    def get_action(self, obs: Dict[str, np.ndarray], info: Dict[str, Any]) -> int:
        ...
        if self.questions_asked_count < self.max_questions:
            # MODIFIED: Instead of just returning a signal, we now generate a question
            return self._ask_question(obs, info)
        else:
            # The final step is to make a recommendation
            return self._choose_recommendation(obs, info)

    def _ask_question(self, obs: Dict[str, np.ndarray], info: Dict[str, Any]) -> int:
        """
        NEW: Generate a single, context-aware follow-up question using an LLM.
        """
        ...
        try:
            ...
            # Store the generated question so the environment can retrieve it
            self.last_response = response.strip()
        except Exception as e:
            print(f"[WARN] Failed to generate a question: {e}")
            self.last_response = "What is your budget?" # Fallback question

        # Return the action index that signals to the environment to ask a question
        return num_products

    def _choose_recommendation(self, obs: Dict[str, np.ndarray], info: Dict[str, Any]) -> int:
        """
        Choose a product using the flexible, "prompting tricks" template.
        ...
        prompt_template = PROMPTS.get("baseline_4_tricks")
        if not prompt_template:
            raise ValueError("Could not find 'baseline_4_tricks' prompt in prompts.yaml")
        ...
        try:
            ...
            if thinking_match:
                self.last_thinking_block = thinking_match.group(1).strip()
            ...
            if json_start_index != -1:
                ...
                if id_val is not None:
                 ...
                 print(f"[WARN] No JSON object found in LLM response. Content: {content}")

        except (json.JSONDecodeError, TypeError, ValueError) as e:
            print(f"[WARN] Failed to parse LLM output. Error: {e}. Content: {content}")
    ...
    def update_preferences(self, episode_result: Dict[str, Any]):
        ...
        self.current_env = None
        pass
# --- Helper functions for checkpointing, identical to experiment1.py ---
...
def run_baseline_prompting_tricks(
    persona_index: int, 
    ...
    """
    Run the 'Prompting Tricks' baseline with checkpointing capabilities,
    structured identically to Experiment 1 for fair comparison.
    """
    
    print(f"=== Baseline: Prompting Tricks (with Checkpointing) ===")
    ...
    if checkpoint_file and os.path.exists(checkpoint_file):
        ...
        all_results, category_results, _ = load_checkpoint(checkpoint_file) # agent_state is ignored for stateless baselines
    ...
    def is_category_relevant_for_persona(category, persona_index, min_score_threshold):
        """Checks if a category is relevant for a specific persona."""
        from .core.user_model import UserModel
        try:
    ...
    for category in selected_categories:
        ...
        for episode in range(episodes_per_category):
            ...
            try:
                ...
                while not terminated and not truncated and step_count < (max_questions + 5): # Safety limit
                ...
                episode_result = {
                    ...
                    'final_info': info, 'full_dialog': full_dialog, 'product_info': product_info, 'thinking_block': agent.last_thinking_block
                }
                
                all_results.append(episode_result)
                category_results[category].append(episode_result)
                agent.update_preferences(episode_result) # Resets agent's internal state for next episode
    ...
    for category, results in category_results.items():
        ...
        regrets = [r['final_info'].get('regret', 100) for r in results if 'regret' in r['final_info']]
        ...
        if scores:
            ...
            print(f"  {category}: Avg Score: {avg_score:.1f}, Avg Regret: {avg_regret:.1f}, Top1 Rate: {top1_rate:.1%}, Episodes: {len(scores)}")

    # (Learning progression is less relevant for a stateless baseline, but kept for structural consistency)
    print("\nProgression (Score):") 
    for category, results in category_results.items():
        if len(results) > 1:
            scores = [r['final_info'].get('chosen_score', 0) for r in results if 'chosen_score' in r['final_info']]
            ...
            print(f"  {category}: {first_half:.1f} -> {second_half:.1f} (Δ{second_half - first_half:+.1f})")

    episode_regrets = [r['final_info'].get('regret') for r in all_results if 'regret' in r.get('final_info', {})]
    
    model_safe_name = model.replace("/", "_").replace(":", "_")
    feedback_safe_name = feedback_type.replace(" ", "_")
    final_results_file = os.path.join(output_dir, f"baseline_prompting_tricks_final_{model_safe_name}_{feedback_safe_name}.json")
    
    with open(final_results_file, 'w') as f:
        json.dump({
            'experiment': 'Baseline 4: LLM With Prompting Tricks',
