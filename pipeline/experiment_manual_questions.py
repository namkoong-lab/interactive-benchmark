#!/usr/bin/env python3
"""
Manual Questions Experiment

This experiment allows you to:
1. Set a specific persona and category
2. Manually create questions for the persona to answer
3. Choose when to end the question phase
4. Have the recommender agent choose a product based on the Q&A

This is a single-episode experiment with one fixed category and persona.
"""
...
- You must recommend exactly one product"""
        ...
        try:
            ...
            # Parse recommendation - accept index or product id
            if "RECOMMEND:" in response:
                import re
                rec_text = response.split("RECOMMEND:")[-1].strip()
                # Allow formats like: "7", "index=7", "id=108800", "product_id: 108800"
                # Try to capture explicit index= first
                ...
                if idx_match:
                    try:
                        ...
                        pass
                # Otherwise capture first integer
                num_match = re.search(r"(\d+)", rec_text)
                if num_match:
                    try:
                        candidate_num = int(num_match.group(1))
                        # If within index range, treat as index
                        if 0 <= candidate_num < num_products:
                            return candidate_num
                        # Otherwise attempt to map as product id -> index
...
def run_manual_questions_interactive(
    ...
    # Create feedback system and persona agent
    ...
    # Check category relevance and prepare cached scores
    try:
    ...
    # Interactive loop to collect dialog
    ...
    try:
        while True:
            try:
                ...
                # End if piped input ends
                break
            if not user_q:
                continue
            if user_q == exit_key:
                break
            # Get persona's answer using the user model
    ...
    # Create agent with collected Q&A and make a recommendation
    ...
    try:
        env = RecoEnv(
            ...
            max_questions=0,  # We already conducted Q&A interactively
            categories=[category],
            agent=agent,
        ...
        # Make recommendation based on provided Q&A
        ...
        # Enhance final_info with rank and total products
...
def run_manual_questions_experiment(
    ...
    # Create agent with provided Q&A
    agent = ManualQuestionsAgent(
        model=model,
        questions_and_answers=questions_and_answers
    )
    ...
    try:
        env = RecoEnv(
            persona_index=persona_index,
            max_questions=0,  # No questions - we provide them manually
            categories=[category],  
            agent=agent,
            feedback_system=feedback_system,
            cached_scores=cached_scores
