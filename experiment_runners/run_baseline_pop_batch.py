...
def run_batch_experiments():
    ...
    experiment_script_path = os.path.join('experiments', 'baseline_popularity.py')
    ...
    for i, seed in enumerate(seeds):
        ...
        output_dir = (
            f"baseline_popularity_exp1_results_{config['num_categories']}_{seed}"
        )
        output_dirs.append(output_dir)

        command = [
            # 'python','-m', 'pipeline.experiment1',
            'python', '-m', 'experiments.baseline_popularity',
    ...
    results_filename = f"baseline_popularity_final_popularity_none.json"
    ...
    # --- Write summary file ---
    summary_filename = (
        f"baseline_popularity_exp1_summary_{config['num_categories']}.json"
    )
