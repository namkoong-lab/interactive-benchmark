...
This script executes pipeline/experiment2.py multiple times with different seeds,
...
def run_batch_experiments():
    ...
    # experiment_script_path = os.path.join('pipeline', 'experiment2.py')
    experiment_script_path = os.path.join('experiments', 'variable_persona.py')
    ...
    for i, seed in enumerate(seeds):
        ...
        output_dir = (
            f"experiment2_results_{config['num_personas']}_{config['model']}_"
            f"{config['feedback_type']}_{config['context_mode']}_"
            f"{config['prompting_tricks']}_{seed}"
        )
        output_dirs.append(output_dir)

        command = [
            # 'python','-m', 'pipeline.experiment2',
            'python', '-m', 'experiments.variable_persona',
            '--num_personas', str(config['num_personas']),
            '--episodes_per_persona', str(config['episodes_per_persona']),
    ...
    results_filename = f"experiment2_results_{config['model']}_{config['feedback_type']}.json"
    ...
    # --- Write summary file ---
    summary_filename = (
        f"experiment2_summary_{config['num_personas']}_{config['model']}_"
        f"{config['feedback_type']}_{config['context_mode']}_"
